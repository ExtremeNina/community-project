...
@Service
@Slf4j
public class CreateCenterServiceImpl implements CreateCenterService {
    ...
    public void draftArticle(ArticleDTO articleDTO) {
        ...
        try {
            Article article = createNewArticle(articleDTO, 0L, 1);
            saveArticleLabels(article.getId(), mergeLabels(articleDTO.getLabel(), articleDTO.getCustomLabels()));
    ...
    public List<ArticleVO> selectDraft() {
        ...
        List<Article> articleList = articleMapper.selectDraftsByUserId(userId);
    ...
    public List<ArticleVO> queryReview() {
        Long userId = SecurityUtils.getCurrentUserId();
        return selectKindOfArticle(userId, 0);
    }

    // 查询未通过文章
    @Override
    public List<ArticleVO> queryRejected() {
        Long userId = SecurityUtils.getCurrentUserId();
    ...
    public List<ArticleVO> selectAll() {
        Long userId = SecurityUtils.getCurrentUserId();
        List<Article> articleList = articleMapper.selectAllByUserId(userId);
        return articleDataConverterUtils.convertToArticleVOList(articleList);
    }
     ...
     * @param status 文章状态 (0:待审核, 1:正常, 2:拦截, 3:人工复核)
     * @return 创建或更新后的文章实体
     */
    private Article createOrUpdateArticle(ArticleDTO articleDTO, Long status) {
        ...
        if (isUpdate) {
            ...
            articleAndLabelMapper.deleteByArticleId(articleDTO.getId());
        } else {
     ...
     * @param articleDTO 文章数据传输对象
     * @param status 文章状态
     * @param isDraft 是否草稿 (0:否, 1:是)
     * @return 新创建的文章实体
     */
    private Article createNewArticle(ArticleDTO articleDTO, Long status, Integer isDraft) {
    ...
    private List<Long> mergeLabels(List<Long> labelIds, List<String> customLabelNames) {
        List<Long> allLabelIds = new ArrayList<>();
        if (labelIds != null && !labelIds.isEmpty()) {
            allLabelIds.addAll(labelIds);
        }
        if (customLabelNames != null && !customLabelNames.isEmpty()) {
            allLabelIds.addAll(processCustomLabels(customLabelNames));
        }
        return allLabelIds;
    }

    private List<Long> processCustomLabels(List<String> customLabelNames) {
        List<Long> newLabelIds = new ArrayList<>();
        for (String name : customLabelNames) {
            String trimmedName = name.trim();
            if (trimmedName.isEmpty()) {
                continue;
            }
            Label existing = labelMapper.selectByName(trimmedName);
            if (existing != null) {
                newLabelIds.add(existing.getId());
            } else {
    ...
    private List<Map<String, Object>> selectArticleLabelMaps(Long articleId) {
        List<ArticleAndLabel> list = articleAndLabelMapper.selectAll(articleId);
        List<Long> labelIdList = list.stream()
                .map(ArticleAndLabel::getLabelId)
                .collect(Collectors.toList());
    ...
    // ============================================================
    // 通用编辑回显接口（支持草稿/待审核/已发布三种状态）
    // ============================================================
    @Override
    public Map<String, Object> getArticleForEdit(Long articleId) {
        SecurityUtils.validateAuthentication();
        log.info("========== 编辑回显开始 ==========");
        log.info("回显请求 - 文章ID: {}", articleId);

        // 1. 查询文章（不限制状态）
        Article article = articleMapper.selectById(articleId);
        if (article == null) {
            log.warn("回显失败 - 文章不存在，ID: {}", articleId);
        ...
        Long currentUserId = SecurityUtils.getCurrentUserId();
        if (!article.getAuthorId().equals(currentUserId)) {
            log.warn("回显失败 - 无权编辑，当前用户: {}, 文章作者: {}", currentUserId, article.getAuthorId());
            throw new SecurityException("无权编辑此文章");
        }

        String statusText = switch (article.getStatus().intValue()) {
            case 1 -> "草稿";
            case 2 -> "待审核";
            case 3 -> "已发布";
            default -> "未知(" + article.getStatus() + ")";
        };
        log.info("文章状态: {} (status={}), 标题: {}", statusText, article.getStatus(), article.getTitle());

        // 3. 获取文章标签（含 id 和 name，前端可区分已有标签和自定义标签）
        List<Map<String, Object>> labelList = selectArticleLabelMaps(articleId);
        log.info("文章标签: {}", labelList);
        ...
        log.info("回显数据 - id: {}, title: {}, coverImageUrl: {}, categoryId: {}, label: {}",
                article.getId(), article.getTitle(), article.getCoverImageUrl(),
                article.getCategoryId(), labelList);
        log.info("========== 编辑回显结束 ==========");
