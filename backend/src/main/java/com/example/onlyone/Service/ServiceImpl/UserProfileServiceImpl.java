...
public class UserProfileServiceImpl implements UserProfileService {
    ...
    // 根据用户ID获取用户信息
    @Override
    public UserProfileVO getUserMessageByUserId(Long userId) {
        //根据用户ID查询用户信息
        ...
        try {
            ...
            if (currentUserId != null && !currentUserId.equals(userId)) {
                Boolean isFollowing = followService.isFollowing(userId,currentUserId);
                userProfileVO.setIsFollow(isFollowing);
            } else {
                userProfileVO.setIsFollow(false);
            }
        } catch (Exception e) {
        ...
        BeanUtils.copyProperties(user, userProfileVO);
        log.info("getUserMessageByUserId userProfileVO:{}", userProfileVO);
        return userProfileVO;
    }
}
