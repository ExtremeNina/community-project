...
public class DynamicServiceImpl implements DynamicService {
    ...
    public List<DyArticleVO> getNewArticles() {
        ...
        if (CollectionUtils.isEmpty(articleIdSet)) {
            ...
            return getLatestArticlesFromFollowedAuthors(userId);
        }
    ...
    //查询关注作者的最新文章（时间流为空时的fallback）
    private List<DyArticleVO> getLatestArticlesFromFollowedAuthors(Long userId) {
        log.info("为用户 {} 查询关注作者的最新文章（时间流为空）", userId);
        ...
        List<Article> latestArticles = new java.util.ArrayList<>();
        ...
        if (CollectionUtils.isEmpty(latestArticles)) {
            log.info("用户 {} 关注的作者没有发布过任何文章", userId);
            return List.of();
        }

        log.info("为用户 {} 查询到 {} 篇关注作者的最新文章", userId, latestArticles.size());
        return convertToVOList(latestArticles);
    }
