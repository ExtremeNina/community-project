...
const connectWs = (token: string) => {
  ...
  ws.value.onclose = () => {
    ...
    if (userInfo.value != null && userInfo.value != undefined) {
      if (retryCount < maxRetries) {
        ...
        setTimeout(() => connectWs(token), retryDelay);
      } else {
  ...
  ws.value.onmessage = (e: any) => {
    ...
    if (message.senderId !== undefined && message.content !== undefined) {
      imStore.setPrivateMessage(message);
...
const updateFriendLastMessage = (message: any) => {
  const friendList = imStore.userList;
  const idx = friendList.findIndex((item: any) => item.uid === String(message.senderId));
  if (idx !== -1) {
    friendList[idx].content = message.content;
    friendList[idx].time = message.dateTime;
    imStore.setUserList([...friendList]);
  }
};
...
const handleModerationResult = (message: any) => {
  const { data } = message;
  const statusText = data.status === "approved"
    ...
    ? "未通过审核"
    : "进入人工复核";
  const typeText = data.targetType === "article" ? "笔记" : "评论";

  ElMessage({
    message: `您的${typeText}${statusText}${data.reason ? "：" + data.reason : ""}`,
    type: data.status === "approved" ? "success" : data.status === "rejected" ? "error" : "warning",
    duration: 5000,
  });

  if (data.status === "rejected" || data.status === "approved") {
    imStore.setModerationResult(data);
  }
};
