<template>
  <div class="note-detail-mask" @click.self="close">
          ...
          <div class="note-scroller" ref="noteScroller" @scroll="loadMoreData">
            <div class="note-content">
              ...
              <div class="desc">
                ...
                <a class="tag tag-search" v-for="(item, index) in noteInfo.tagList" :key="index">#{{ item }}</a>
              </div>
...
import { saveCommentByDTO } from "@/api/comment";
  ...
  () => {
    ...
    if (props.nid !== null && props.nid !== "") {
      getNoteById(props.nid).then((res: any) => {
        ...
        // 后端 ArticleDetailVO 转前端 NoteInfo
        const profile = article.userProfileVO || {};
        noteInfo.value = {
          id: String(article.id || ''),
          title: article.title || '',
          content: article.content || '',
          noteCover: article.coverImageUrl || '',
          uid: String(profile.userId || article.author || ''),
          username: profile.username || article.author || '',
          avatar: profile.icon || '',
          imgList: article.coverImageUrl ? [article.coverImageUrl] : [],
          type: 1,
          likeCount: article.loveCount || 0,
          collectionCount: 0,
          commentCount: article.comments || 0,
          tagList: article.label || [],
          time: article.publishTime || '',
          isFollow: profile.isFollow || false,
          isLike: article.isLoved || false,
          isCollection: false,
          pinned: '0',
        };
