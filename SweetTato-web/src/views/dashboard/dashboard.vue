...
import { getRecommendNote, getRecommendFeed, getNoteByDTO, addRecord } from "@/api/search";
import { getAllCategories } from "@/api/createCenter";
...
const loadMoreData = () => {
  if (categoryClass.value === '0') {
    ...
    currentPage.value += 1;
    getNoteByDTO(noteDTO.value).then((res) => {
      setData(res);
    });
  }
};
...
const getFeedList = () => {
  ...
  const userId = userInfo && userInfo.id ? Number(userInfo.id) : undefined;
  getRecommendFeed({
    userId,
    offset: feedOffset.value,
    limit: 20,
  }).then((res: any) => {
    const { hasMore, notes } = res.data;
    feedHasMore.value = hasMore;
    isEnd.value = !hasMore;
    const mapped = (notes || []).map((item: any) => ({
      id: item.articleId,
      title: item.title || '',
      src: item.coverImageUrl || '',
      noteCover: item.coverImageUrl || '',
      username: item.author || '未知',
      likeCount: item.loveCount || 0,
      isLike: false,
      avatar: '',
      noteCoverHeight: 200,
    }));
    if (feedOffset.value === 0) {
      noteList.value = mapped;
    } else {
      noteList.value.push(...mapped);
    }
  });
};
