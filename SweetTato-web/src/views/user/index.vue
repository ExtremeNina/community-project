<template>
  <div class="user-page">
            ...
            <div class="basic-info">
              <div class="user-basic">
                ...
                <div class="user-content">
                  <span class="user-redId">小甜薯号：{{ userInfo.hsId }}</span
    ...
    <div class="feeds-tab-container" style="--1ee3a37c: all 0.4s cubic-bezier(0.2, 0, 0.25, 1) 0s">
      <div v-if="type === 1 && uid === currentUid" class="article-status-tabs">
        <div class="status-item" :class="articleStatus === '' ? 'active' : ''" @click="onArticleStatusChange('')">
          全部 · {{ articleStatusCounts.all }}
        </div>
        <div class="status-divider"></div>
        <div class="status-item" :class="articleStatus === '1' ? 'active' : ''" @click="onArticleStatusChange('1')">
          草稿 · {{ articleStatusCounts.draft }}
        </div>
        <div class="status-divider"></div>
        <div class="status-item" :class="articleStatus === '2' ? 'active' : ''" @click="onArticleStatusChange('2')">
          审核中 · {{ articleStatusCounts.pending }}
        </div>
        <div class="status-divider"></div>
        <div class="status-item" :class="articleStatus === '3' ? 'active' : ''" @click="onArticleStatusChange('3')">
          已通过 · {{ articleStatusCounts.published }}
        </div>
        <div class="status-divider"></div>
        <div class="status-item" :class="articleStatus === '0' ? 'active' : ''" @click="onArticleStatusChange('0')">
          未通过 · {{ articleStatusCounts.rejected }}
        </div>
      </div>
      <Chat
        v-if="chatShow"
      ...
      <Note :type="type" :articleStatus="articleStatus" :key="articleStatusKey"> </Note>
    </div>
  </div>
</template>
...
import { getUserById, getUserInfoByToken, updateUser } from "@/api/user";
import { getAllArticles } from "@/api/createCenter";
...
const articleStatusKey = ref(0);
const articleStatusCounts = ref({ all: 0, draft: 0, pending: 0, published: 0, rejected: 0 });

const onArticleStatusChange = (status: string) => {
  articleStatus.value = status;
  articleStatusKey.value++;
};
...
const initData = () => {
  const profilePromise = uid === currentUid.toString()
    ? getUserInfoByToken()
    : getUserById(uid);
  ...
  profilePromise.then((res) => {
    ...
    userInfo.value = {
      ...
      followerCount: userData.follows,
      fanCount: userData.fan,
      trendCount: 0,
    };
    _isFollow.value = userData.isFollow || false;
    // 自己看自己 → 用完整信息更新 store
    if (uid === currentUid.toString()) {
      const storeUser = userStore.getUserInfo();
      storeUser.username = userData.username;
      storeUser.description = userData.description;
      storeUser.followerCount = userData.follows;
      storeUser.fanCount = userData.fan;
      userStore.setUserInfo(storeUser);
    }
  });
  ...
  if (uid === currentUid.toString()) {
    getAllArticles().then((res) => {
      ...
      const draftCount = articles.filter((a: any) => a.status === 1).length;
      const pendingCount = articles.filter((a: any) => a.status === 2).length;
      const publishedCount = articles.filter((a: any) => a.status === 3).length;
      const rejectedCount = articles.filter((a: any) => a.status === 0).length;
      articleStatusCounts.value = {
        all: articles.length,
        draft: draftCount,
