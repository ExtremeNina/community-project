      ...
      <div class="parent-comment" v-for="(oneComment, oneIndex) in dataList" :key="oneIndex">
            ...
            <div class="right">
              ...
              <div class="info">
                ...
                <div class="interactions">
                  ...
                  <div class="reply" @click="loadTwoMore(oneComment.id)">
                    <span class="like-wrapper">
                      <span>共{{ oneComment.children.length }}条回复</span>
                    </span>
                  </div>
        ...
        <div class="reply-container">
          ...
          <div 
            ...
            v-if="oneComment.children.length > 0 && !expandedMap.get(oneComment.id)"
            @click="loadTwoMore(oneComment.id)"
          >
            共{{ oneComment.children.length }}条评论，点击展开
          </div>
          <!-- 收起按钮 -->
          <div 
            class="reply-count-btn" 
            v-if="oneComment.children.length > 0 && expandedMap.get(oneComment.id)"
            @click="reback(oneComment.id)"
          >
            收起
          </div>
          <!-- 子评论列表 -->
          <div class="list-container" v-if="expandedMap.get(oneComment.id)">
...
const twoIndex = ref<string | number>(-1);
const expandedMap = reactive(new Map()); // 存储每个父评论的展开状态

const likeComment = (comment: any, status: number, one: number, two: string | number) => {
...
const saveComment = (comment: any, one: number, two: string | number) => {
  oneIndex.value = one;
  twoIndex.value = two;
...
const addComment = () => {
  ...
  if (comment.pid === "0") {
    ...
    dataList.value[oneIndex.value].children.splice(Number(twoIndex.value) + 1, 0, comment);
  }
  computedTotal.value += 1;
};

const loadTwoMore = (oneCommentId: number) => {
  expandedMap.set(oneCommentId, true);
};

const reback = (oneCommentId: number) => {
  expandedMap.set(oneCommentId, false);
};
...
onMounted(() => {
  console.log("Comment component mounted, nid:", props.nid);
  getCommentData();
});
</script>
